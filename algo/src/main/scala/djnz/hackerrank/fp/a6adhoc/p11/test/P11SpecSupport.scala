...
import djnz.hackerrank.util.ResourceAccess
import org.scalatest.funsuite.AnyFunSuite
import org.scalatest.matchers.should.Matchers

trait P11SpecSupport extends AnyFunSuite with Matchers with ResourceAccess {
  ...
  def parseQuery(raw: String): Query =
    Solution.parseUnsafe(raw) match {
      case q: Query => q
      case other    => failX(s"expected Query, got $other")
    }
